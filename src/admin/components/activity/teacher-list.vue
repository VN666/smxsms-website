<template>
	<div class="teacher-list">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '德育活动', '班主任专栏', '列表']"></h-breadcrumb>
		</div>

		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('teacher-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		requestData () {
			...
			this.$http({
        		method: "post",
        		url: this.$api.activity_teacher_query,
        		data: {
		...
		goEdit (id) {
			this.$router.push({ name: "teacherEdit", params: { id: id }});
		},
		beforeDel (row, index) {
		...
		deleteRow (row, index) {
        	...
        	this.$http({
        		method: "post",
        		url: this.$api.activity_teacher_del,
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.activity_teacher_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
