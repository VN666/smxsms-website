<template>
	<div class="teacher-edit">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '德育活动', '班主任专栏', '编辑']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		async submit () {
			...
			this.$http({
				method: "post",
				url: this.$api.activity_teacher_edit,
				data: this.addForm
			}).then((res) => {
				this.isSaving = false;
		...
		requestData (id) {
			this.$http({
				method: "post",
				url: this.$api.activity_teacher_queryById,
...
<style>
	.teacher-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.teacher-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.teacher-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
