<template>
	<div class="health-list">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '德育活动', '健康教育', '列表']"></h-breadcrumb>
		</div>

		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('health-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		requestData () {
			...
			this.$http({
        		method: "post",
        		url: this.$api.activity_health_query,
        		data: {
		...
		goEdit (id) {
			this.$router.push({ name: "healthEdit", params: { id: id }});
		},
		beforeDel (row, index) {
		...
		deleteRow (row, index) {
        	...
        	this.$http({
        		method: "post",
        		url: this.$api.activity_health_del,
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.activity_health_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
