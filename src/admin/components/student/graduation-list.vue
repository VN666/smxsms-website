<template>
	<div class="graduation-list">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '学生天地', '毕业留影', '列表']"></h-breadcrumb>
		</div>

		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('graduation-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		requestData () {
			...
			this.$http({
        		method: "post",
        		url: this.$api.student_graduation_query,
        		data: {
		...
		goEdit (id) {
			this.$router.push({ name: "graduationEdit", params: { id: id }});
		},
		beforeDel (row, index) {
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.student_graduation_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
