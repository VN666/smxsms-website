<template>
	<div class="exhibition-add">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '学生动态', '学生作品', '新增']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		goBack () {
			this.$router.push({ path: "exhibition-list" });
		},
		fileUpload () {
		...
		async submit () {
			...
			if (upload.code === 200) {
				this.$http({
					method: "post",
					url: this.$api.student_exhibition_add,
					data: this.addForm
				}).then((res) => {
					this.isSaving = false;
					if (res.code === 200) {
...
<style>
	.exhibition-add .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.exhibition-add .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.exhibition-add .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
