<template>
	<div class="flag-edit">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '学生天地', '纪律卫生', '编辑']"></h-breadcrumb>
		</div>
	...
	methods: {
		...
		async submit () {
			...
			if (upload.code === 200) {
				this.$http({
					method: "post",
					url: this.$api.student_flag_edit,
					data: this.addForm
				}).then((res) => {
					this.isSaving = false;
					if (res.code === 200) {
		...
		requestData (id) {
			this.$http({
				method: "post",
				url: this.$api.student_flag_queryById,
				data: {
					id: id,
					addViews: false
				}
...
<style>
	.flag-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.flag-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.flag-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
