<template>
	<div class="staff-list">
		...
		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('staff-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		requestData () {
			...
			this.$http({
        		method: "post",
        		url: this.$api.union_staff_query,
        		data: {
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.union_staff_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
