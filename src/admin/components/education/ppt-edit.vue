<template>
	<div class="ppt-edit">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '教学科研', '教学课件', '编辑']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		goBack () {
			this.$router.push({ path: "ppt-list" });
		},
		beforeSubmit () {
		...
		async submit () {
			...
			if (upload.code === 200) {
				this.$http({
					method: "post",
					url: this.$api.education_ppt_edit,
					data: this.addForm
				}).then((res) => {
					this.isSaving = false;
					if (res.code === 200) {
		...
		requestData (id) {
			this.$http({
				method: "post",
				url: this.$api.education_ppt_queryById,
				data: {
					id: id,
					addViews: false
				}
...
<style>
	.ppt-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.ppt-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.ppt-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
