<template>
	<div class="feature-add">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '教学科研', '特色教育', '新增']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		async submit () {
			...
			this.$http({
				method: "post",
				url: this.$api.education_feature_add,
				data: this.addForm
			}).then((res) => {
				this.isSaving = false;
...
<style>
	.feature-add .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.feature-add .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.feature-add .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
