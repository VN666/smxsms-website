<template>
	<div class="case-list">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '教学科研', '教学案例', '列表']"></h-breadcrumb>
		</div>

		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('case-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		goEdit (id) {
			this.$router.push({ name: "caseEdit", params: { id: id }});
		},
		beforeDel (row, index) {
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.education_case_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
