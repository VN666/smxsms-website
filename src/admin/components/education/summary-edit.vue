<template>
	<div class="summary-edit">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '教学科研', '教学反思', '编辑']"></h-breadcrumb>
		</div>
	...
	methods: {
		...
		async submit () {
			...
			if (upload.code === 200) {
				this.$http({
					method: "post",
					url: this.$api.education_summary_edit,
					data: this.addForm
				}).then((res) => {
					this.isSaving = false;
					if (res.code === 200) {
		...
		requestData (id) {
			this.$http({
				method: "post",
				url: this.$api.education_summary_queryById,
				data: {
					id: id,
					addViews: false
				}
...
<style>
	.summary-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.summary-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.summary-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
