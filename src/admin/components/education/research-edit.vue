	...
	methods: {
		...
		async submit () {
			...
			if (upload.code === 200) {
				this.$http({
					method: "post",
					url: this.$api.education_research_edit,
					data: this.addForm
				}).then((res) => {
					this.isSaving = false;
					if (res.code === 200) {
		...
		requestData (id) {
			this.$http({
				method: "post",
				url: this.$api.education_research_queryById,
				data: {
					id: id,
					addViews: false
				}
...
<style>
	.research-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.research-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.research-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
