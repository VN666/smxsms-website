<template>
	<div class="research-add">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '教学科研', '教研动态', '新增']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		goBack () {
			this.$router.push({ path: "research-list" });
		},
		async submit () {
			this.isSaving = true;
			this.$http({
				method: "post",
				url: this.$api.education_research_add,
				data: this.addForm
			}).then((res) => {
				this.isSaving = false;
...
<style>
	.research-add .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.research-add .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.research-add .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
