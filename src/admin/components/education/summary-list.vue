<template>
	<div class="summary-list">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '教学科研', '教学反思', '列表']"></h-breadcrumb>
		</div>

		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('summary-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		requestData () {
			...
			this.$http({
        		method: "post",
        		url: this.$api.education_summary_query,
        		data: {
		...
		goEdit (id) {
			this.$router.push({ name: "summaryEdit", params: { id: id }});
		},
		beforeDel (row, index) {
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.education_summary_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
