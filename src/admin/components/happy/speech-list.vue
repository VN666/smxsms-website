<template>
	<div class="speech-list">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '幸福教育', '幸福感言', '列表']"></h-breadcrumb>
		</div>

		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('speech-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		requestData () {
			this.$http({
        		method: "post",
        		url: this.$api.happy_speech_query,
        		data: {
		...
		goEdit (id) {
			this.$router.push({ name: "speechEdit", params: { id: id }});
		},
		beforeDel (row, index) {
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.happy_speech_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
