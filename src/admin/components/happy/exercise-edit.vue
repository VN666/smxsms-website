<template>
	<div class="exercise-edit">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '幸福教育', '幸福教育活动', '编辑']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		goBack () {
			this.$router.push({ path: "exercise-list" });
		},
		beforeSubmit () {
		...
		async submit () {
			...
			if (upload.code === 200) {
				this.$http({
					method: "post",
					url: this.$api.happy_exercise_edit,
					data: this.addForm
				}).then((res) => {
					this.isSaving = false;
					if (res.code === 200) {
...
<style>
	.exercise-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.exercise-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.exercise-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
