<template>
	<div class="idea-edit">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '幸福教育', '幸福理念', '编辑']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		goBack () {
			this.$router.push({ path: "idea-list" });
		},
		beforeSubmit () {
		...
		requestData (id) {
			this.$http({
				method: "post",
				url: this.$api.happy_idea_queryById,
				data: {
					id: id,
					addViews: false
				}
...
<style>
	.idea-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.idea-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.idea-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
