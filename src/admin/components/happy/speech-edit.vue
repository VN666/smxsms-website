<template>
	<div class="speech-edit">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '幸福教育', '幸福感言', '编辑']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		goBack () {
			this.$router.push({ path: "speech-list" });
		},
		beforeSubmit () {
		...
		async submit () {
			...
			this.$http({
				method: "post",
				url: this.$api.happy_speech_edit,
				data: this.addForm
			}).then((res) => {
				this.isSaving = false;
		...
		requestData (id) {
			this.$http({
				method: "post",
				url: this.$api.happy_speech_queryById,
...
<style>
	.speech-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.speech-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.speech-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
