<template>
	<div class="knowledge-add">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '家长学校', '家教知识', '新增']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		async submit () {
			...
			this.$http({
				method: "post",
				url: this.$api.parent_knowledge_add,
				data: this.addForm
			}).then((res) => {
				this.isSaving = false;
...
<style>
	.knowledge-add .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.knowledge-add .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.knowledge-add .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
