<template>
	<div class="knowledge-list">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '家长学校', '家教知识', '列表']"></h-breadcrumb>
		</div>

		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('knowledge-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		requestData () {
			this.$http({
        		method: "post",
        		url: this.$api.parent_knowledge_query,
        		data: {
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.parent_knowledge_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
