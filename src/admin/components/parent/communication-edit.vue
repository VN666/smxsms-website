<template>
	<div class="communication-edit">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '家长学校', '学校沟通', '编辑']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		goBack () {
			this.$router.push({ path: "communication-list" });
		},
		bmit () {
		...
		async submit () {
			...
			if (upload.code === 200) {
				this.$http({
					method: "post",
					url: this.$api.parent_communication_edit,
					data: this.addForm
				}).then((res) => {
					this.isSaving = false;
					if (res.code === 200) {
...
<style>
	.communication-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.communication-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.communication-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
