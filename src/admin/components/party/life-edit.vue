<template>
	<div class="life-edit">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '学校党建', '组织生活', '编辑']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		goBack () {
			this.$router.push({ path: "life-list" });
		},
		beforeSubmit () {
		...
		requestData (id) {
			this.$http({
				method: "post",
				url: this.$api.party_life_queryById,
...
<style>
	.life-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.life-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.life-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
