<template>
	<div class="position-list">
		...
		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('position-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		requestData () {
			...
			this.$http({
        		method: "post",
        		url: this.$api.party_position_query,
        		data: {
		...
		deleteRow (row, index) {
        	...
        	this.$http({
        		method: "post",
        		url: this.$api.party_position_del,
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.party_position_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
