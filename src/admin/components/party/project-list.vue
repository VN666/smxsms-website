<template>
	<div class="project-list">
		...
		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('project-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		requestData () {
			...
			this.$http({
        		method: "post",
        		url: this.$api.party_project_query,
        		data: {
		...
		deleteRow (row, index) {
        	...
        	this.$http({
        		method: "post",
        		url: this.$api.party_project_del,
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.party_project_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
