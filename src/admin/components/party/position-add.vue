<template>
	<div class="position-add">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '学校党建', '阵地建设', '新增']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		async submit () {
			...
			this.$http({
				method: "post",
				url: this.$api.party_position_add,
				data: this.addForm
			}).then((res) => {
				this.isSaving = false;
...
<style>
	.position-add .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.position-add .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.position-add .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
