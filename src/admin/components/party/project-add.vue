<template>
	<div class="project-add">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '二中党建', '活动项目', '新增']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		async submit () {
			...
			this.$http({
				method: "post",
				url: this.$api.party_project_add,
				data: this.addForm
			}).then((res) => {
				this.isSaving = false;
...
<style>
	.project-add .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.project-add .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.project-add .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
