<template>
	<div class="classroom-list">
		...
		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('classroom-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		requestData () {
			...
			this.$http({
        		method: "post",
        		url: this.$api.establish_classroom_query,
        		data: {
		...
		deleteRow (row, index) {
        	...
        	this.$http({
        		method: "post",
        		url: this.$api.establish_classroom_del,
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.establish_classroom_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
