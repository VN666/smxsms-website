x<template>
	<div class="practice-list">
		...
		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('practice-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		requestData () {
			...
			this.$http({
        		method: "post",
        		url: this.$api.establish_practice_query,
        		data: {
		...
		goEdit (id) {
			this.$router.push({ name: "practiceEdit", params: { id: id }});
		},
		beforeDel (row, index) {
		...
		deleteRow (row, index) {
        	...
        	this.$http({
        		method: "post",
        		url: this.$api.establish_practice_del,
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.establish_practice_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
