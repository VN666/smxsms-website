<template>
	<div class="volunteer-edit">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '文明创建', '志愿者风采', '编辑']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		goBack () {
			this.$router.push({ path: "volunteer-list" });
		},
		beforeSubmit () {
		...
		async submit () {
			...
			this.$http({
				method: "post",
				url: this.$api.establish_volunteer_edit,
				data: this.addForm
			}).then((res) => {
				this.isSaving = false;
		...
		requestData (id) {
			this.$http({
				method: "post",
				url: this.$api.establish_volunteer_queryById,
...
<style>
	.volunteer-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.volunteer-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.volunteer-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
