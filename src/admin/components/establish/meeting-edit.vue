<template>
	<div class="meeting-edit">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '二中创建', '创建会议', '编辑']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		async submit () {
			...
			this.$http({
				method: "post",
				url: this.$api.establish_meeting_edit,
				data: this.addForm
			}).then((res) => {
				this.isSaving = false;
		...
		requestData (id) {
			this.$http({
				method: "post",
				url: this.$api.establish_meeting_queryById,
...
<style>
	.meeting-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.meeting-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.meeting-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
