<template>
	<div class="meeting-list">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '文明创建', '创建会议', '列表']"></h-breadcrumb>
		</div>

		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('meeting-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		requestData () {
			...
			this.$http({
        		method: "post",
        		url: this.$api.establish_meeting_query,
        		data: {
		...
		deleteRow (row, index) {
        	...
        	this.$http({
        		method: "post",
        		url: this.$api.establish_meeting_del,
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.establish_meeting_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
