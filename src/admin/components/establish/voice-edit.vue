...
export default {
	...
	methods: {
		...
		async submit () {
			...
			this.$http({
				method: "post",
				url: this.$api.establish_voice_edit,
				data: this.addForm
			}).then((res) => {
				this.isSaving = false;
		...
		requestData (id) {
			this.$http({
				method: "post",
				url: this.$api.establish_voice_queryById,
...
<style>
	.voice-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.voice-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.voice-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
