<template>
	<div class="volunteer-add">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '文明创建', '志愿者风采', '新增']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		async submit () {
			...
			this.$http({
				method: "post",
				url: this.$api.establish_volunteer_add,
				data: this.addForm
			}).then((res) => {
				this.isSaving = false;
...
<style>
	.volunteer-add .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.volunteer-add .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.volunteer-add .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
