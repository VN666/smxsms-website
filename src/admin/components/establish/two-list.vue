<template>
	<div class="two-list">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '文明创建', '结对帮扶', '列表']"></h-breadcrumb>
		</div>

		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('two-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		requestData () {
			...
			this.$http({
        		method: "post",
        		url: this.$api.establish_two_query,
        		data: {
		...
		goEdit (id) {
			this.$router.push({ name: "twoEdit", params: { id: id }});
		},
		beforeDel (row, index) {
		...
		deleteRow (row, index) {
        	...
        	this.$http({
        		method: "post",
        		url: this.$api.establish_two_del,
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.establish_two_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
