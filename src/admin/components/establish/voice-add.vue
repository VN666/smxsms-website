<template>
	<div class="voice-add">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '文明创建', '文明之声', '新增']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		goBack () {
			this.$router.push({ path: "voice-list" });
		},
		async submit () {
			this.isSaving = true;
			this.$http({
				method: "post",
				url: this.$api.establish_voice_add,
				data: this.addForm
			}).then((res) => {
				this.isSaving = false;
...
<style>
	.voice-add .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.voice-add .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.voice-add .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
