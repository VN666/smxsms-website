<template>
	<div class="practice-add">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '文明创建', '新时代文明实践活动', '新增']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		async submit () {
			...
			this.$http({
				method: "post",
				url: this.$api.establish_practice_add,
				data: this.addForm
			}).then((res) => {
				this.isSaving = false;
...
<style>
	.practice-add .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.practice-add .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.practice-add .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
