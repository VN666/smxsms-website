<template>
	<div class="guarantee-edit">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '为您服务', '后勤保障', '编辑']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		goBack () {
			this.$router.push({ path: "guarantee-list" });
		},
		beforeSubmit () {
		...
		async submit () {
			...
			if (upload.code === 200) {
				this.$http({
					method: "post",
					url: this.$api.service_guarantee_edit,
					data: this.addForm
				}).then((res) => {
					this.isSaving = false;
					if (res.code === 200) {
		...
		requestData (id) {
			this.$http({
				method: "post",
				url: this.$api.service_guarantee_queryById,
				data: {
					id: id,
					addViews: false
				}
...
<style>
	.guarantee-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.guarantee-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.guarantee-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
