<template>
	<div class="computer-add">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '为您服务', '电教知识', '新增']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		goBack () {
			this.$router.push({ path: "computer-list" });
		},
		fileUpload () {
		...
		async submit () {
			...
			if (upload.code === 200) {
				this.$http({
					method: "post",
					url: this.$api.service_computer_add,
					data: this.addForm
				}).then((res) => {
					this.isSaving = false;
					if (res.code === 200) {
...
<style>
	.computer-add .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.computer-add .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.computer-add .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
