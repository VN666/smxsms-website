<template>
	<div class="party-edit">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '党团工会', '党建动态', '编辑']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		goBack () {
			this.$router.push({ path: "party-list" });
		},
		beforeSubmit () {
		...
		requestData (id) {
			this.$http({
				method: "post",
				url: this.$api.group_party_queryById,
				data: {
					id: id,
					addViews: false
				}
...
<style>
	.party-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.party-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.party-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
