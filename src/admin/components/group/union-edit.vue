...
export default {
	...
	methods: {
		...
		requestData (id) {
			this.$http({
				method: "post",
				url: this.$api.group_union_queryById,
				data: {
					id: id,
					addViews: false
				}
...
<style>
	.union-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.union-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.union-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
