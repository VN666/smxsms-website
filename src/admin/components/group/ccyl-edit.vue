<template>
	<div class="ccyl-edit">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '团建活动', '编辑']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		...
		goBack () {
			this.$router.push({ path: "ccyl-list" });
		},
		beforeSubmit () {
		...
		requestData (id) {
			this.$http({
				method: "post",
				url: this.$api.group_ccyl_queryById,
...
<style>
	.ccyl-edit .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.ccyl-edit .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.ccyl-edit .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
