<template>
	<div class="excellent-list">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '党团工会', '创优争先', '列表']"></h-breadcrumb>
		</div>

		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('excellent-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		requestData () {
			this.$http({
        		method: "post",
        		url: this.$api.group_excellent_query,
        		data: {
		...
		goEdit (id) {
			this.$router.push({ name: "excellentEdit", params: { id: id }});
		},
		beforeDel (row, index) {
		...
		deleteRow (row, index) {
        	...
        	this.$http({
        		method: "post",
        		url: this.$api.group_excellent_del,
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.group_excellent_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
