<template>
	<div class="enroll-list">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '新闻动态', '招生信息', '列表']"></h-breadcrumb>
		</div>

		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('enroll-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
		...
		requestData () {
			this.$http({
        		method: "post",
        		url: this.$api.news_enroll_query,
        		data: {
		...
		goEdit (id) {
			this.$router.push({ name: "enrollEdit", params: { id: id }});
		},
		beforeDel (row, index) {
		...
		changeIsTop (id, isTop, timecreate) {
			this.$http({
        		method: "post",
        		url: this.$api.news_enroll_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
