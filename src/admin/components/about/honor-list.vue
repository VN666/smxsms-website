<template>
	<div class="honor_list">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '关于二中', '学校荣誉', '列表']"></h-breadcrumb>
		</div>

		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('honor-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
...
export default {
	...
	methods: {
        ...
        goEdit (id) {
        	this.$router.push({ name: "honorEdit", params: { id: id }});
        },
        changeIsTop (id, isTop, timecreate) {
        	this.$http({
        		method: "post",
        		url: this.$api.about_honor_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
