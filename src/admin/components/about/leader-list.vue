<template>
	<div class="leader_list">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '关于二中', '学校领导', '列表']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		moveUp (row, index) {
			this.$http({
				method: "post",
				url: this.$api.about_leader_move,
				data: {
		...
		submit () {
			...
			this.$http({
				method: "post",
				data: this.addForm,
				url: this.dialogEdit ? this.$api.about_leader_edit : this.$api.about_leader_add
			}).then((res) => {
				this.isSaving = false;
				if (res.code === 200) {
					this.requestData();
