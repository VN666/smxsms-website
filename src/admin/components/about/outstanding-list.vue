<template>
	<div class="outstanding_list">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '关于二中', '名师风采', '列表']"></h-breadcrumb>
		</div>
...
export default {
	...
	methods: {
		moveUp (row, index) {
			this.$http({
				method: "post",
				url: this.$api.about_outstanding_move,
				data: {
		...
		requestData () {
			...
			this.$http({
				method: "post",
				url: this.$api.about_outstanding_query,
				data: {
		...
		submit () {
			...
			this.$http({
				method: "post",
				data: this.addForm,
				url: this.dialogEdit ? this.$api.about_outstanding_edit : this.$api.about_outstanding_add
			}).then((res) => {
				this.isSaving = false;
				if (res.code === 200) {
					this.requestData();
		...
		deleteRow (row, index) {
			...
			this.$http({
				method: "post",
				url: this.$api.about_outstanding_del,
		...
		goEdit (row, index) {
			this.clearForm();
			this.$http({ method: "post", url: this.$api.about_outstanding_queryById, data: { id: row.id }}).then((res) => {
