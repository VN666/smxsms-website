<template>
	<div class="media-list">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '新闻动态', '媒体报道', '列表']"></h-breadcrumb>
		</div>

		<div class="option_wrap" ref="option_wrap">
			<el-button type="success" size="mini" @click="goPath('media-add')"><i class="el-icon-plus el-icon--left"></i>新增</el-button>
				...
				<el-row>
					<el-col :span="4">
						<el-form-item label="标题">
							<el-input v-model="filters.headline" placeholder="标题" size="mini" clearable></el-input>
						</el-form-item>
					</el-col>
					<el-col :span="4">
		...
		<div class="table_wrap" :style="{height:tableMaxHeight+'px'}" stripe>
			<el-table :data="tableData" height="100%" stripe :header-cell-style="{background:'#F5F5F5',color:'#606266'}">
				...
				<el-table-column label="原始链接" prop="link" min-width="150" show-overflow-tooltip></el-table-column>
...
export default {
	...
	methods: {
        ...
        goEdit (id) {
        	this.$router.push({ name: "mediaEdit", params: { id: id }});
        },
        changeIsTop (id, isTop, timecreate) {
        	this.$http({
        		method: "post",
        		url: this.$api.news_media_changeIsTop,
        		data: {
        			id: id,
        			isTop: isTop,
