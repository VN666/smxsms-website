<template>
	<div class="display-add">
		<div class="breadcrumb_wrap" ref="breadcrumb_wrap">
			<h-breadcrumb :bread="['后台管理', '七彩校园', '活动报道', '新增']"></h-breadcrumb>
		</div>
	...
	methods: {
		...
		async submit () {
			...
			if (upload.code === 200) {
				this.$http({
					method: "post",
					url: this.$api.school_display_add,
					data: this.addForm
				}).then((res) => {
					this.isSaving = false;
					if (res.code === 200) {
...
<style>
	.display-add .file-upload .el-upload-list {
		display: flex;
		flex-direction: row;
		justify-content: flex-start;
		flex-wrap: wrap;
	...
	.display-add .file-upload .el-upload-list li {
		margin-right: 8px;
		width: 200px;
		background: #F5F7FA;
	}
	.display-add .file-upload .el-upload-list li:first-child {
		margin-top: 0px;
	}
</style>
