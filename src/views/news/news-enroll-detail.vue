	...
	<div class="enrollDetail">
		...
		<div class="detail-download" v-if="content.fileListSrc.length">
			...
			<div class="download-file">
				<div class="download-item" v-for="(item, index) in content.fileListSrc" @click="downLoad(item)"><img src="@/src/assets/icons/download-icon.png" />{{ item.match(/news\/(\S*)(\s*)-oss-/)[1] + "." + $utils.getFileExt(item) }}</div>
			</div>
		</div>
		<video src="https://www.smxsdezx.cn/public/video/错位时空.mp4" controls width="1008">
		</video>
	</div>
</template>
