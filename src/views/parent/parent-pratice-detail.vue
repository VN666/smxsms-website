	...
	<div class="praticeDetail">
		...
		<div class="detail-download" v-if="content.fileListSrc.length">
			...
			<div class="download-file">
				<div class="download-item" v-for="(item, index) in content.fileListSrc" @click="downLoad(item)"><img src="@/src/assets/icons/download-icon.png" />{{ item.match(/parent\/(\S*)(\s*)-oss-/)[1] + "." + $utils.getFileExt(item) }}</div>
			</div>
		</div>
	</div>
</template>
